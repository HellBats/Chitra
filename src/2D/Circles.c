        ...
        (circle.pos_x+circle.radius),(circle.pos_y+circle.radius),&x,&y,&x_,&y_))
    {
        for(int j=2*y;j<2*y_;j+=2)
        {
            for(int i=2*x;i<2*x_;i+=2)
            {
                ...
                uint32_t transparency = (popularity==4?(color_trans << 24):((color_trans/4*popularity) << 24)) 
                | 0x00000000;
                uint32_t new_color = MixRGBA(chitra.pixels[j/2*chitra.stride+i/2],((color << 8) >> 8) | transparency);
